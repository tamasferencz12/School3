...
// Task 1
const atlantaToCharlotte = railServices.some(
  (service) =>
    service.startCity === "Atlanta" && service.endCity === "Charlotte"
);
console.log(atlantaToCharlotte);
document.querySelector("#task-1").innerText = atlantaToCharlotte
  ? "Van"
  : "Nincs";
...
document.querySelector("#task-5-gomb").addEventListener("click", (event) => {
  ...
  document.querySelectorAll("#task-34 tbody tr.kivalasztva").forEach((sor) => {
    console.log(sor);
    sum += parseInt(sor.dataset.price);
    amount++;
  });

  document.querySelector("#task-5-span").innerText =
