...
`(:)` előtt: hozzáadtunk az első elemhez egyet.<br/>
...
Rekurzív függvény
: Olyan függvény, amely önmaga definiálásához saját magát használja fel.

Még egy példa, matematikában elcsépelt példa a rekurzióra: faktoriális<br/>
...
HELYES KÓD: Nem érdemes a sima `length` függvényt használni, mert az csak `Int`-et ad vissza, amelyről megtanultuk,
            véges méretű, értékei -2^63^ ─ 2^63^-1 között vannak.

Példa feladat, ahol el tudja rontani a rossz függvény használata a megoldást:<br/>
Definiáljuk egy másik módon a faktoriális függvényt:<br/>
...
A probléma ott van, hogy az `Int` korlátos, a `length` pedig csak `Int`-et ad vissza. Mivel 21! > 2^63^-1,
ezért túlcsordulás történik, ami azt jelenti, hogy negatív lesz az eredmény.
Ezt ki is lehet próbálni: `(maxBound :: Int) + 1` eredménye egy negatív szám lesz. `(maxBound :: Int)` == 2^63^-1
...
*: Ha van sok `'a'`, akkor egy 2^63^ elemű listával lehet elrontani a length függvényt, hiszen túlcsordulás miatt a `length` eredménye egy negatív szám lesz. Ennek az eredményét ne várjuk meg, mert valahol több ezer év lesz, mire visszaadja az üres listát arra.
